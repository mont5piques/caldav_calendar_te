...
//  private $driver;
  ...
  {
    if ($calendars === null) {
      $calendars = $driver->list_calendars(calendar_driver::FILTER_PERSONAL | calendar_driver::FILTER_WRITEABLE);
    }
  ...
  */
  ...
  function calendar_action()
  {
    ...
    if ($success)
      ...
      $error_msg = $this->gettext('errorsaving') . ($driver->last_error ? ': ' . $driver->last_error :'');
      $this->rc->output->show_message($error_msg, 'error');
    }
  ...
  function event_action()
  {
    ...
    switch ($action) {
      case "new":
        ...
        else if ($success = $driver->new_event($event)) {
          $event['id'] = $event['uid'];
          $event['_savemode'] = 'all';
      ...
      case "edit":
        ...
        else if ($success = $driver->edit_event($event)) {
          $this->cleanup_event($event);
      ...
      case "resize":
        ...
        else if ($success = $driver->resize_event($event)) {
          $this->event_save_success($event, $old, $action, $success);
        }
      ...
      case "move":
        ...
        else if ($success = $driver->move_event($event)) {
          $this->event_save_success($event, $old, $action, $success);
        }
...
//      $chars = "!# abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ 1234567890";
  ...
  public function freebusy_status()
  {
    ...
    $fblist = $driver->get_freebusy_list($email, $start, $end);
      ...
      if ($invitation = $itip->get_invitation($token)) {
        ...
        // save submitted RSVP status
          ...
          if ($this->rc->user->ID) {
            ...
            $existing   = $driver->get_event($this->event);
            ...
            if (!$existing && ($calendar = $this->get_default_calendar($invitation['event']['sensitivity']))) {
              ...
              && ($calendar = $driver->get_calendar($existing['calendar']))
            ) {
              $this->event       = $invitation['event'];
              ...
              if ($driver->edit_event($this->event))
...
//      $headers = $imap->get_message_headers($uid);
  ...
  public function user_delete($args)
  {
    ...
    foreach($this->get_drivers() as $driver)
     ...
     $this->setup();
     $this->load_driver();
     return $driver->user_delete($args);
  }
